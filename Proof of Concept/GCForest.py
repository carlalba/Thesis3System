...
class gcForest(object):
    ...
    def __init__(self, 
             ...
             min_samples_mgs=0.1,
             min_samples_cascade=0.1,
             cascade_layer=float("inf"),   # <<< ADD THIS LINE
             tolerance=0.0,
             n_jobs=-1,
             enable_mgs=True):   # <<< NEW
        ...
        if isinstance(window, int):
            ...
            self.window = window # Catches 'None' and explicitly sets the attribute

        setattr(self, 'stride', stride) # Keep the rest as is
        ...
        setattr(self, 'enable_mgs', enable_mgs)
        self.enable_mgs = enable_mgs
    ...
    def fit(self, X, y):
        ...
        # -------------------------------------------------------
        # FIX: automatically adjust window size for UMAP-reduced data
        # -------------------------------------------------------
        sequence_length = X.shape[1]   # number of UMAP components

        if hasattr(self, "window") and self.window is not None:
            adjusted_window = [min(w, sequence_length - 1) for w in self.window]
        
            # update the model’s window list
            self.window = adjusted_window

        print(f"[gcForest] Adjusted window sizes → {self.window} (sequence length = {sequence_length})")
        # -------------------------------------------------------

        if self.enable_mgs:
            mgs_X = self.mg_scanning(X, y)
            X_input = np.hstack((X, mgs_X))
        else:
            mgs_X = X
            X_input = X
